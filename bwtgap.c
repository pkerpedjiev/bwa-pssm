						  ...
						  bwt_width_t *seed_width, const gap_opt_t *opt, int *_n_aln, gap_stack_t *stack)
{
	...
	while (stack->n_entries) {
        ...
        //ooga
		//fprintf(stderr, "#1\t[%d,%d,%d,%c]\t[%d,%d,%d][%d]\t[%u,%lu]\t[%lu,%lu]\t%d\n", stack->n_entries, i, seq[i], "MID"[e.state], e.n_mm, e.n_gapo, e.n_gape, max_diff, width[i-1].bid, width[i-1].w, k, l, e.last_diff_pos);
    ...
    //fprintf(stderr, "finished, stack->n_entries: %d\n", stack->n_entries);
	*_n_aln = n_aln;
