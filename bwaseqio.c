...

{
	...
	while ((l = kseq_read(seq)) >= 0) {
		...
		if (seq->qual.l) { // copy quality
			...
			p->rqual = (ubyte_t*)strdup((char*)seq->qual.s);
			if (trim_qual >= 1) n_trimmed += bwa_trim_read(trim_qual, p);
		} 

        //remove starting and ending wildcard characters
		/*
        for (i = 0; i < p->len; i++) 
            if (p->seq[i] < 4)
                break;
		...
		*/
        ...
        //TODO: possible bug if end == start
        //p->full_len = end - start + 1;
        //p->len = p->full_len;
	//memcpy(p->rseq, p->seq+start, p->len);
    ...
    if (l == -3) 
        fprintf(stderr, "Wrong number of entries on one of the PSSM rows.\n");
    else if (l == -4)
        fprintf(stderr, "Wrong number of PSSM rows.\n");
...
{
    return bwa_read_pssm_seq(bs, n_needed, n, mode, trim_qual, NULL, NULL, NULL);
}
